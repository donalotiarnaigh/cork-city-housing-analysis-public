...
# Set paths
ppr_raw_path <- here(here("data/raw/ppr/PPR-2024-Cork.csv"))
ppr_spatial_path <- here(here("data/processed/final/ppr_corkCity.gpkg"))
ppr_output_path <- here(here("data/processed/final/ppr_corkCity_with_dates.gpkg"))
...
# Set paths
airbnb_raw_path <- here(here("data/raw/airbnb/listings.csv"))
airbnb_spatial_path <- here(here("data/processed/final/airbnb_corkCity_with_price.gpkg"))
airbnb_output_path <- here(here("data/processed/final/airbnb_corkCity_with_dates.gpkg"))
...
for (i in 1:length(app_lines)) {
  ...
  if (grepl('ppr_data.*st_read.*ppr_corkCity\\.gpkg', app_lines[i])) {
    app_lines[i] <- 'ppr_data <- st_read(here(here("data/processed/final/ppr_corkCity_with_dates.gpkg")), quiet = TRUE)'
  }
  
  # Update Airbnb dataset path
  if (grepl('airbnb_data.*st_read.*airbnb_corkCity_with_price\\.gpkg', app_lines[i])) {
    app_lines[i] <- 'airbnb_data <- st_read(here(here("data/processed/final/airbnb_corkCity_with_dates.gpkg")), quiet = TRUE)'
  }
}
