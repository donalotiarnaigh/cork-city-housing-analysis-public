...
ppr_geocoded <- st_read(file.path(base_path, "data/processed/ppr_geocoded.gpkg"))
...
# Save cleaned data
st_write(ppr_clean, 
         file.path(base_path, "data/processed/ppr_geocoded_clean.gpkg"),
         delete_layer = TRUE)

# Save non-spatial version
ppr_clean_csv <- ppr_clean %>%
  st_drop_geometry() %>%
  write_csv(file.path(base_path, "data/processed/ppr_geocoded_clean.csv"))

# Print summary
cat("\nCleaned geocoded PPR data saved:\n")
cat(sprintf("1. GeoPackage: %s\n", file.path(base_path, "data/processed/ppr_geocoded_clean.gpkg")))
cat(sprintf("2. CSV: %s\n", file.path(base_path, "data/processed/ppr_geocoded_clean.csv")))
...
write_csv(summary_report, 
          file.path(base_path, "data/processed/ppr_geocoding_summary.csv"))
